...
int builtin_pipes(int argc, char **argv);
...
int builtin_waitone(int argc, char **argv) {
  ...
  for (int i = 0; i < num_exit_statuses; i++) {
    if (exit_statuses[i].pid == target_pid ||
        (target_pid == -1 && num_exit_statuses > 0)) {
      lastExitStatus = WEXITSTATUS(exit_statuses[i].status);
      memmove(&exit_statuses[i], &exit_statuses[i + 1],
              (num_exit_statuses - i - 1) * sizeof(ChildExitStatus));
      num_exit_statuses--;
      return 0;
    }
  }
...
int builtin_pipes(int argc, char **argv) {
  ...
  int num_stages = argc - 1;
  int pipes_fd[MAX_TOKENS][2];
  ...
  for (int stage = 0; stage < num_stages; stage++) {
    ...
    if (pids[stage] == 0) {
      ...
      if (stage > 0) {
        if (dup2(pipes_fd[stage - 1][0], STDIN_FILENO) == -1) {
          perror("dup2 stdin");
          exit(127);
        }
      }
      ...
      if (stage < num_stages - 1) {
        if (dup2(pipes_fd[stage][1], STDOUT_FILENO) == -1) {
          perror("dup2 stdout");
          exit(127);
        }
      }
      ...
      char *stage_command = strdup(argv[stage + 1]);
      if (!stage_command) {
        exit(127);
      }

      char *stage_argv[MAX_TOKENS + 1];
      int stage_argc = 0;
      char *token = strtok(stage_command, " \t\n");
      while (token != NULL && stage_argc < MAX_TOKENS) {
        stage_argv[stage_argc] = strdup(token);
        stage_argc++;
        token = strtok(NULL, " \t\n");
      }
      stage_argv[stage_argc] = NULL;
      free(stage_command);
      ...
      struct Command *cmd = getCommand(stage_argv[0]);
      if (cmd != NULL) {

        int result = cmd->handler(stage_argc, stage_argv);
        for (int i = 0; i < stage_argc; i++) {
          free(stage_argv[i]);
        }
        exit(result);
      } else {

        execvp(stage_argv[0], stage_argv);
        perror("exec");
        for (int i = 0; i < stage_argc; i++) {
          free(stage_argv[i]);
        }
...
int main() {
  ...
  FILE *shell_input = fdopen(original_stdin, "r");
  if (shell_input == NULL) {
    perror("fdopen shell input");
    exit(1);
  }
  ...
  while (1) {
    ...
    char *result;
    do {
      result = fgets(inputLine, sizeof(inputLine), shell_input);
    ...
    static char inputFile[MAX_LINE_LENGTH];
    static char outputFile[MAX_LINE_LENGTH];

    for (int i = commandTokenCount - 1; i >= 1; i--) {
      struct Token token = tokens[i];
      if (token.start[0] == '>' && token.length > 1) {
        const char *filename = token.start + 1;
        while (*filename && isspace(*filename)) {
          filename++;
        }
        int filename_len = token.length - (filename - token.start);
        strncpy(outputFile, filename, filename_len);
        outputFile[filename_len] = '\0';
        outputRedirect = outputFile;
        for (int j = i; j < commandTokenCount - 1; j++) {
          tokens[j] = tokens[j + 1];
        }
        commandTokenCount--;
      } else if (token.start[0] == '<' && token.length > 1) {
        const char *filename = token.start + 1;
        while (*filename && isspace(*filename)) {
          filename++;
        }
        int filename_len = token.length - (filename - token.start);
        strncpy(inputFile, filename, filename_len);
        inputFile[filename_len] = '\0';
    ...
    if (debugLevel > 0) {
      if (inputRedirect) {
        fprintf(stderr, "Input redirect: '%.*s'\n",
                (int)(strlen(inputRedirect)), inputRedirect);
      }
      if (outputRedirect) {
        fprintf(stderr, "Output redirect: '%.*s'\n",
                (int)(strlen(outputRedirect)), outputRedirect);
      }
      if (runInBackground) {
        fprintf(stderr, "Background: 1\n");
      }
    }
    ...
    if (command != NULL) {
      ...
      if (debugLevel > 0) {
        fprintf(stderr, "Executing builtin '%s' in %s\n", commandName,
      ...
      if (runInBackground) {
        ...
        if (pid == -1) {
          ...
          int cmd_result = command->handler(commandTokenCount, argv);
          exit(cmd_result);
        } else {
          result = 0;
        }
        ...
        if (inputRedirect != NULL) {
          ...
          if (saved_stdin == -1) {
            perror("dup stdin");
            result = 1;
            goto cleanup_builtin;
          }

          input_fd = open(inputRedirect, O_RDONLY);
          if (input_fd == -1) {
            perror("open input");
            result = 1;
            goto cleanup_builtin;
        ...
        if (outputRedirect != NULL) {
          ...
          output_fd = open(outputRedirect, O_WRONLY | O_CREAT | O_TRUNC, 0644);
          if (output_fd == -1) {
            perror("open output");
            result = 1;
            goto cleanup_builtin;
        ...
        result = command->handler(commandTokenCount, argv);
      ...
      if (debugLevel > 0) {
        ...
        fprintf(stderr, "External command '%s'\n", externalCommand);
        lastExitStatus = 0;
      }
...
void sigchld_handler(int sig) {
  ...
  while ((pid = waitpid(-1, &status, WNOHANG)) > 0) {
    if (WIFEXITED(status) || WIFSIGNALED(status)) {
      exit_statuses = realloc(exit_statuses, (num_exit_statuses + 1) *
                                                 sizeof(ChildExitStatus));
